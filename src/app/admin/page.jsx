...
export default function AdminDashboard() {
  ...
  // Protect admin route: Redirect if not logged in or not an admin
  useEffect(() => {
    if (!authLoading) {
  ...
  // Fetch comprehensive admin data when component mounts or tab changes
  useEffect(() => {
  ...
  const updateReservationStatus = async (id, status) => {
    try {
      ...
      setReservations(
        reservations.map((res) => (res._id === id ? { ...res, status } : res)),
      );
  ...
  const handleSubmit = async (e) => {
    ...
    const toastId = toast.loading(
      editingRoomId ? "Updating room..." : "Creating room...",
    );
    ...
    try {
      ...
      if (editingRoomId) {
        ...
        setRooms(
          rooms.map((r) =>
            r._id === editingRoomId || r.id === editingRoomId ? updatedRoom : r,
          ),
        );
      } else {
  ...
  const handleMenuSubmit = async (e) => {
    ...
    setIsSubmitting(true);
    const toastId = toast.loading(
      editingMenuId ? "Updating menu item..." : "Creating menu item...",
    );
    ...
    try {
      ...
      let res;
      if (editingMenuId) {
        res = await api.put(`/menu/${editingMenuId}`, data, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        setMenus(
          menus.map((m) => (m._id === editingMenuId ? res.data.menuItem : m)),
        );
        toast.success("Menu item updated!", { id: toastId });
      } else {
        res = await api.post("/menu", data, {
          headers: { "Content-Type": "multipart/form-data" },
        });
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-12">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-4 h-32"
                >
            ...
            <div className="bg-white rounded-2xl border border-gray-100 min-h-[500px] p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
                {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                  <div
                    key={i}
                    className="h-64 bg-gray-50 rounded-2xl border border-gray-100"
                  ></div>
                ))}
              </div>
            </div>
  ...
  // ----------------------------------------
  // Main Render: Admin Dashboard Layout
  // ----------------------------------------
  return (
    <div className="flex min-h-screen bg-slate-50/50">
      {/* Sidebar: Navigation tabs for admin workspace */}
      <AdminSidebar
        activeTab={activeTab}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">
                Manage your properties, bookings, and services seamlessly.
              </p>
            </div>
          ...
          >
            {loadingData ? (
              ...
              <>
                {/* Render corresponding tab component based on activeTab state */}
                {activeTab === "rooms" && (
                  <AdminRoomsTab
                    rooms={rooms}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {[
                        ...
                        <label
                          key={item}
                          className={`flex items-center space-x-2 cursor-pointer px-4 py-2 rounded-xl border transition-colors ${amenities.includes(item) ? "bg-[#0f284f]/5 border-[#0f284f]/30" : "bg-white border-slate-200 hover:bg-slate-50"}`}
                        >
                          <input
                            type="checkbox"
                          ...
                          />
                          <span
                            className={`text-sm font-medium ${amenities.includes(item) ? "text-[#0f284f]" : "text-slate-600"}`}
                          >
                            {item}
                          </span>
                        </label>
