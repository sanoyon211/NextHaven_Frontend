      ...
      {reservations.length > 0 ? (
        <div className="space-y-6">
          {reservations.map((res) => (
            <div
              key={res._id}
              className="flex flex-col md:flex-row md:items-start justify-between border border-gray-100 rounded-sm p-4 md:p-6 hover:shadow-md transition-shadow"
            >
              <div>
                <span className="text-xs font-bold text-gray-400 uppercase tracking-widest block mb-1">
                  Reservation #{res._id.substring(0, 8).toUpperCase()}
                </span>
                <h3 className="text-xl font-bold text-[#0f284f] uppercase tracking-wide mb-2">
