  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 p-4 md:p-6 bg-slate-50/30">
      ...
      {menus.map((menu) => (
        <div
          key={menu._id}
          className="bg-white rounded-xl shadow-sm border border-slate-200/60 flex flex-col hover:shadow-md transition-shadow overflow-hidden group"
        >
          <div className="h-40 overflow-hidden relative bg-gray-100">
            ...
            <div className="absolute bottom-3 right-3">
              <button
                ...
                className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold transition-all border backdrop-blur-sm shadow-sm ${
                  menu.isAvailable
          ...
          <div className="p-5 flex-1 flex flex-col">
            <div className="flex justify-between items-start gap-2 mb-2">
              <h3
                className="text-[#0f284f] font-bold text-base truncate"
                title={menu.name}
              >
                {menu.name}
              </h3>
