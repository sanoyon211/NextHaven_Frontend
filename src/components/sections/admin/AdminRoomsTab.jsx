    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 p-4 md:p-6 bg-slate-50/30">
      ...
      {rooms.map((room) => (
        <div
          ...
          className="bg-white shadow-sm hover:shadow-xl rounded-lg overflow-hidden group border border-gray-100 transition-all relative"
        >
          <div className="h-48 md:h-60 overflow-hidden relative bg-gray-100">
            ...
            <div className="absolute top-4 right-4 flex gap-2">
              <button
                ...
                className="p-2 bg-white/90 backdrop-blur-sm text-slate-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg shadow-sm transition-colors"
              >
                <Edit className="w-4 h-4" />
              </button>
              <button
                onClick={() => handleDeleteRoom(room._id || room.id)}
                className="p-2 bg-white/90 backdrop-blur-sm text-slate-600 hover:text-rose-600 hover:bg-rose-50 rounded-lg shadow-sm transition-colors"
              >
            ...
            <div className="absolute bottom-4 right-4">
              {room.isOccupiedToday ? (
                ...
                <button
                  ...
                  className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold transition-all border backdrop-blur-sm ${
          ...
          <div className="p-5 md:p-6 flex flex-col flex-1">
            <h3
              className="text-[#0f284f] font-extrabold uppercase text-base md:text-lg mb-1 md:mb-2 truncate"
              title={room.title}
            >
              {room.roomNumber
            ...
            <div className="flex justify-between items-end mt-4 md:mt-6 pt-4 border-t border-slate-100 mt-auto">
              <p className="text-gray-500 text-xs w-1/2 leading-relaxed font-medium">
                {room.capacity ? `${room.capacity} adults` : "2 adults"} /{" "}
