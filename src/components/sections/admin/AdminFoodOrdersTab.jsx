...
export default function AdminFoodOrdersTab({
  foodOrders,
      ...
      {foodOrders.map((order) => (
        ...
        >
          <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-3">
            ...
            <span
              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${order.orderStatus === "delivered" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : order.orderStatus === "cancelled" ? "bg-rose-50 text-rose-700 border-rose-200" : "bg-amber-50 text-amber-700 border-amber-200"}`}
            >
              {order.orderStatus}
            </span>
          </div>
          <div className="flex-1 space-y-4 mb-4">
            <div>
              ...
              {order.orderNotes && (
                <p className="text-xs text-slate-500 mt-1.5 italic bg-slate-50 p-2 rounded-lg border border-slate-100 flex gap-1 items-start">
                  <FileText className="w-3.5 h-3.5 mt-0.5 shrink-0" /> {order.orderNotes}
                </p>
              )}
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {order.items.map((i) => (
                  <span
                    key={i._id || i.name}
                    className="inline-block bg-white text-slate-700 px-2 py-1 rounded text-xs font-medium border border-slate-200 shadow-sm"
                  >
                    {i.quantity}x {i.name}
                  </span>
                ))}
