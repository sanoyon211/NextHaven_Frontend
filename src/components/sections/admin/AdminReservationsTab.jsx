        ...
        >
          <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-3">
            <span className="font-mono text-sm font-bold text-[#0f284f]">
              #{res._id.substring(0, 8).toUpperCase()}
            </span>
            <span
              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${res.status === "confirmed" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : res.status === "cancelled" ? "bg-rose-50 text-rose-700 border-rose-200" : "bg-amber-50 text-amber-700 border-amber-200"}`}
            >
              {res.status}
            </span>
          </div>
          <div className="flex-1 space-y-4 mb-4">
            <div>
              ...
              <p className="text-sm font-bold text-slate-900">
                {res.user?.name || res.name || "Unknown"}
              </p>
              <p className="text-xs font-medium text-slate-600 mt-1 flex items-center gap-1.5">
                <Mail className="w-4 h-4 text-slate-400 shrink-0" />{" "}
                {res.user?.email || res.email || "N/A"}
              </p>
              <p className="text-xs font-medium text-slate-600 mt-1 flex items-center gap-1.5">
                <Phone className="w-4 h-4 text-slate-400 shrink-0" /> {res.phone || "N/A"}
              </p>
            </div>
            <div>
              <p className="text-xs text-slate-500 font-medium mb-1 uppercase tracking-wider">
