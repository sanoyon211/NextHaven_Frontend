  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 p-4 md:p-6 bg-slate-50/30">
      ...
      {users.map((u) => (
        <div
          key={u._id}
          className="bg-white rounded-xl shadow-sm border border-slate-200/60 p-5 flex flex-col hover:shadow-md transition-shadow relative overflow-hidden group"
        >
          <div
            className={`absolute top-0 left-0 w-1 h-full transition-colors ${u.role === "admin" ? "bg-purple-500" : "bg-slate-200 group-hover:bg-[#0f284f]"}`}
          ></div>
          <div className="flex items-start gap-4 mb-4 pl-1">
            <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center overflow-hidden shrink-0 border border-slate-200">
              {u.avatar ? (
                ...
                <span className="text-[#0f284f] font-bold text-lg">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p className="text-xs text-slate-500 truncate" title={u.email}>
                {u.email}
              </p>
            </div>
          </div>
