...
export default function Navbar() {
  ...
  const textColorClass =
    isHomePage && !isScrolled ? "text-white" : "text-[#0f284f]";
  ...
  return (
    <header className={headerClasses}>
      <div
        className={`mx-auto flex transition-all duration-300 ${isScrolled ? "h-14" : "h-20"} max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8`}
      >
        {/* Mobile Menu Button */}
        ...
        <nav className="hidden md:flex flex-1 items-center space-x-8 text-sm font-semibold tracking-wide">
          ...
          >
            <button
              className={`flex items-center space-x-1 transition-colors py-2 ${pathname.startsWith("/rooms") ? "text-[#d4af37]" : "hover:text-[#d4af37]"}`}
            >
              <span>OUR ROOMS</span>
              <svg
          ...
          <Link
            href="/restaurant"
            className={`transition-colors ${pathname === "/restaurant" ? "text-[#d4af37]" : "hover:text-[#d4af37]"}`}
          >
            RESTAURANT
          </Link>
          <Link
            href="/about"
            className={`transition-colors ${pathname === "/about" ? "text-[#d4af37]" : "hover:text-[#d4af37]"}`}
          >
            ABOUT US
          </Link>
        </nav>
        ...
        <div className="flex shrink-0 items-center justify-center flex-1 md:flex-none">
          <Link
            href="/"
            className="font-heading text-2xl md:text-3xl font-extrabold tracking-widest whitespace-nowrap"
          >
            NEXT HAVEN
          </Link>
        </div>
        ...
        <div className="hidden md:flex flex-1 items-center justify-end space-x-8 text-sm font-semibold tracking-wide">
          <Link
            href="/contact"
            className={`transition-colors ${pathname === "/contact" ? "text-[#d4af37]" : "hover:text-[#d4af37]"}`}
          >
            CONTACT US
          </Link>
          {mounted ? (
            user ? (
              ...
              >
                <button className="flex items-center focus:outline-none">
                  <div
                    className={`h-10 w-10 rounded-full bg-[#d4af37] overflow-hidden relative flex-shrink-0 border-2 border-transparent transition-colors ${isHomePage && !isScrolled ? "hover:border-white" : "hover:border-[#d4af37]"}`}
                  >
                    {user.avatar ? (
                      <Image
                        src={user.avatar}
              ...
              <Link
                ...
                className={`transition-colors ${pathname === "/login" ? "text-[#d4af37]" : "hover:text-[#d4af37]"}`}
              >
                LOGIN
              </Link>
            )
          ...
          >
            <nav className="flex flex-col px-4 py-6 space-y-4 text-sm font-semibold tracking-wide">
              <Link
                ...
                href="/rooms"
                className={`block py-2 ${pathname.startsWith("/rooms") ? "text-[#d4af37]" : "text-[#0f284f]"}`}
              >
                OUR ROOMS
              </Link>
              <Link
                onClick={() => setIsMobileMenuOpen(false)}
                href="/restaurant"
                className={`block py-2 ${pathname === "/restaurant" ? "text-[#d4af37]" : "text-[#0f284f]"}`}
              >
                RESTAURANT
              </Link>
              <Link
                onClick={() => setIsMobileMenuOpen(false)}
                href="/about"
                className={`block py-2 ${pathname === "/about" ? "text-[#d4af37]" : "text-[#0f284f]"}`}
              >
                ABOUT US
              </Link>
              <Link
                onClick={() => setIsMobileMenuOpen(false)}
                href="/contact"
                className={`block py-2 ${pathname === "/contact" ? "text-[#d4af37]" : "text-[#0f284f]"}`}
              >
                CONTACT US
              </Link>
                  ...
                  (user ? (
                    <>
                      <Link
                        onClick={() => setIsMobileMenuOpen(false)}
                        href="/dashboard"
                        className="block py-2 text-center border border-gray-300 rounded-sm hover:bg-gray-50 transition-colors"
                      >
                        DASHBOARD
                      </Link>
                      {user.role === "admin" && (
                        <Link
                          onClick={() => setIsMobileMenuOpen(false)}
                          href="/admin"
                          className="block py-2 text-center border border-[#0f284f] text-[#0f284f] rounded-sm hover:bg-gray-50 transition-colors font-bold"
                        >
                          ADMIN PANEL
                        </Link>
                      )}
                      <button
                        ...
                        className="block w-full py-2 text-center border border-red-200 text-red-600 rounded-sm hover:bg-red-50 transition-colors"
                      >
                        LOGOUT
                      </button>
                    </>
                    ...
                    <Link
                      ...
                      className="block py-2 text-center border border-[#0f284f] rounded-sm hover:bg-gray-50 transition-colors text-[#0f284f]"
                    >
                      LOGIN
                    </Link>
                  ))}
